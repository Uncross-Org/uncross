...
- **Anyone can void a crossed auction's trades before it settles.** `cancel_and_refund` returns every order's full escrow. It is meant for when normal settlement cannot run, such as when an issuer pauses the mint. But it has no caller check and does not require the mint to be paused, and whoever sends the first settlement batch after a cross fixes which path is used. So a stranger can choose the refund path and undo every trade in that auction. Nothing can be stolen: every order gets back exactly what it locked. The keeper settles within seconds of each cross, which narrows the window but does not close it. The fix is identified (only a paused mint or the auction's operator may choose the refund path) and not yet shipped.
...
▲ **24 Sept evening.** Added the community auction's results, read from chain with `scripts/event-report.mjs`. None of the 11 outside wallets is one of the bot's 42. The event sequence was not run, so the bot was in both books and the text says so. The rent-recovery figure is now the exact 2.199437 SOL from 120 closes. Added the `cancel_and_refund` disclosure, which the docs pass found; if the fix ships before entry, this line changes to "found and fixed" with the test signatures.
