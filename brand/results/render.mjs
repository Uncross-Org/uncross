// Render the results clip: load results.html, call render(t) for every frame at
...
const b = (await measure([LOCK], 9893))["uncross-lockup-light.svg"];
...
const html = fs.readFileSync(path.join(D, "results.html"), "utf8")
  .replace("__BOOK__", fs.readFileSync(path.join(D, "book.json"), "utf8"))
  .replace("__LOCKUP__", JSON.stringify(lockup));
const page = path.join(D, ".results.rendered.html");
...
const chrome = spawn(path.join(cache, dir, "chrome-headless-shell-mac-arm64", "chrome-headless-shell"), ["--remote-debugging-port=9894", "--headless", "--hide-scrollbars", "--force-color-profile=srgb", "about:blank"], { stdio: "ignore" });
let v; for (let i = 0; i < 80 && !v; i++) { try { v = await (await fetch("http://127.0.0.1:9894/json/version")).json(); } catch { await sleep(250); } }
...
if (err !== "function") throw new Error("results.html did not define render(): " + err);
...
const out = path.join(D, "../kit/video/results.mp4");
