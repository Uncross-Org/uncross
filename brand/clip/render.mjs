// Render the launch clip: load clip.html, call render(t) for every frame at
...
const b = (await measure([LOCK], 9881))["uncross-lockup-light.svg"];
...
const chrome = spawn(path.join(cache, dir, "chrome-headless-shell-mac-arm64", "chrome-headless-shell"), ["--remote-debugging-port=9882", "--headless", "--hide-scrollbars", "--force-color-profile=srgb", "about:blank"], { stdio: "ignore" });
let v; for (let i = 0; i < 80 && !v; i++) { try { v = await (await fetch("http://127.0.0.1:9882/json/version")).json(); } catch { await sleep(250); } }
...
if (err !== "function") throw new Error("clip.html did not define render(): " + err);
...
const out = path.join(D, "../kit/video/launch.mp4");
