...
export const LinesGradientShader: React.FC<LinesGradientShaderProps> = ({
  ...
  useEffect(() => {
    ...
    // Resizing the canvas resets its backing store, which clears it. Under
    // reduced motion the draw loop stops after one frame, so without an
    // explicit redraw here a resize leaves the canvas permanently blank —
    // whether you got the still frame or an empty hero was a race.
    const ro = new ResizeObserver(() => {
      resize();
      if (prefersReducedMotionRef.current && !stopped) {
    ...
    const draw = (timestamp: number) => {
      ...
      // If the user prefers reduced motion, hold one still frame. Not phase
      // zero: at elapsed 0 the bands sit outside the visible area, so the
      // canvas renders empty. Freeze at a phase where the composition is
      // actually on screen.
      const STATIC_PHASE = 6.2;
